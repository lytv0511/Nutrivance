...
struct JournalEntry: Identifiable, Codable, Equatable {
    ...
    /// Lowercased person name → richer remembered profile used by future disambiguation.
    var personProfiles: [String: JournalPersonProfile] = [:]
    ...
    init(from decoder: Decoder) throws {
        ...
        personProfiles = try container.decodeIfPresent([String: JournalPersonProfile].self, forKey: .personProfiles) ?? [:]
        if personProfiles.isEmpty, !personRelationshipHints.isEmpty {
            for (name, role) in personRelationshipHints {
                personProfiles[name] = JournalPersonProfile(name: name, relationship: role)
            }
        }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(personProfiles, forKey: .personProfiles)
...
struct JournalPersonProfile: Codable, Equatable, Hashable {
    var name: String
    var relationship: String
    var disambiguationNote: String

    init(name: String, relationship: String, disambiguationNote: String = "") {
        self.name = name
        self.relationship = relationship
        self.disambiguationNote = disambiguationNote
    }
}
...
enum JournalPersistence {
    ...
    /// Same container as CloudKit / entitlements — full journal JSON syncs here (no 1 MB KVS cap).
    private static let ubiquityContainerIdentifier = "iCloud.None.Nutrivance"
    /// NSUbiquitousKeyValueStore rejects values near/above ~1 MB; strip images for KVS only when needed.
    private static let kvsMaxEncodedBytes = 900_000
    ...
    private static var ubiquityJournalDirectoryURL: URL? {
        guard let root = FileManager.default.url(forUbiquityContainerIdentifier: ubiquityContainerIdentifier) else {
            return nil
        }
        return root
            .appendingPathComponent("Documents", isDirectory: true)
            .appendingPathComponent("JournalSync", isDirectory: true)
    }

    private static var ubiquityJournalFileURL: URL? {
        ubiquityJournalDirectoryURL?.appendingPathComponent("journal_entries.json")
    }
    ...
    static func loadEntries() -> [JournalEntry] {
        ...
        let kvsEntries = loadCloudEntries()
        let ubiquityEntries = loadUbiquityEntries()

        var merged = merge(localEntries: localEntries, cloudEntries: kvsEntries)
        merged = merge(localEntries: merged, cloudEntries: ubiquityEntries)

        if !sameEntryCorpus(merged, localEntries) {
            persistLocalEntries(merged)
        }
        if !sameEntryCorpus(merged, kvsEntries) {
            persistCloudKeyValueEntries(merged)
        }
        if !sameEntryCorpus(merged, ubiquityEntries) {
            persistUbiquityEntries(merged)
        }
    ...
    static func persistEntries(_ entries: [JournalEntry]) {
        ...
        persistUbiquityEntries(normalized)
        persistCloudKeyValueEntries(normalized)
    }

    /// Order-agnostic comparison so we don’t rewrite every load when only sort order differs.
    private static func sameEntryCorpus(_ a: [JournalEntry], _ b: [JournalEntry]) -> Bool {
        guard a.count == b.count else { return false }
        let sa = a.sorted { $0.id.uuidString < $1.id.uuidString }
        let sb = b.sorted { $0.id.uuidString < $1.id.uuidString }
        return sa == sb
    }
    ...
    private static func persistCloudKeyValueEntries(_ entries: [JournalEntry]) {
        do {
            var payload = entries
            var data = try JSONEncoder().encode(payload)
            if data.count > kvsMaxEncodedBytes {
                ...
                if data.count > kvsMaxEncodedBytes {
                    print("JournalPersistence: iCloud KVS payload still exceeds \(kvsMaxEncodedBytes) bytes after stripping images; relying on iCloud Drive journal file for full sync.")
                }
            }
            let cloudStore = NSUbiquitousKeyValueStore.default
            ...
            print("Failed to save journal entries to iCloud KVS:", error)
        }
    }

    private static func loadUbiquityEntries() -> [JournalEntry] {
        guard let url = ubiquityJournalFileURL,
        ...
        do {
            ...
            print("JournalPersistence: failed to load iCloud journal file:", error)
            return []
        }
    }

    private static func persistUbiquityEntries(_ entries: [JournalEntry]) {
        guard let dir = ubiquityJournalDirectoryURL, let fileURL = ubiquityJournalFileURL else {
            return
        }
        do {
            ...
            print("JournalPersistence: failed to save journal to iCloud Documents (\(ubiquityContainerIdentifier)):", error)
        }
    }

    /// Async refresh for pull-to-refresh on Mac Catalyst (KVS + ubiquitous file may update shortly after `synchronize()` returns).
...
struct JournalView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onReceiveViewControl(.nutrivanceViewControlNewJournalEntry) {
                currentEntry = JournalEntry()
                showingEditor = true
            }
...
enum InlineSuggestionChipRole: String, Hashable, Codable {
...
struct InlineSuggestion: Identifiable, Hashable {
    ...
    init(id: UUID = UUID(), preview: String, confidence: Double, mode: InlineSuggestionApplyMode, chipRole: InlineSuggestionChipRole = .caretContinuation) {
        self.id = id
...
enum InlineSuggestionApplyMode: Hashable, Codable {
...
@MainActor
final class JournalSuggestionTelemetry {
    static let shared = JournalSuggestionTelemetry()
    private let key = "journal.suggestion.telemetry.v1"
    private let defaults = UserDefaults.standard

    private struct Counters: Codable {
        var shownInline: Int = 0
        var shownNudges: Int = 0
        var acceptedInline: Int = 0
        var acceptedNudges: Int = 0
        var pinnedNudges: Int = 0
        var savedNudges: Int = 0
    }

    private var counters: Counters = .init()
    ...
    func trackShown(inline: Int, nudges: Int) {
        counters.shownInline += max(0, inline)
        counters.shownNudges += max(0, nudges)
        persist()
    }

    func trackAccepted(role: InlineSuggestionChipRole) {
        switch role {
        case .nudge:
            counters.acceptedNudges += 1
        default:
            counters.acceptedInline += 1
        }
        persist()
    }

    func trackPinnedNudge() {
        counters.pinnedNudges += 1
        persist()
    }

    func trackSavedNudge() {
        counters.savedNudges += 1
        persist()
    }
...
struct JournalOutlineSection: Identifiable, Equatable {
    ...
    init(id: UUID = UUID(), label: String, themes: [String], textRange: NSRange) {
        self.id = id
        self.label = label
        self.themes = themes
...
// MARK: - Assistant cache (inline suggestions, nudges, outline, correlations — syncs with journal entry)
...
struct JournalAssistantOutlineSnapshot: Codable, Equatable, Hashable {
    var id: UUID
    var label: String
    var themes: [String]
    var textRangeStartUTF16: Int
    var textRangeLengthUTF16: Int
}

struct JournalAssistantInlineSnapshot: Codable, Equatable, Hashable {
    var id: UUID
    var preview: String
    var confidence: Double
    var chipRoleRaw: String
    var applyMode: InlineSuggestionApplyMode

    init(id: UUID = UUID(), preview: String, confidence: Double, chipRole: InlineSuggestionChipRole, mode: InlineSuggestionApplyMode) {
        self.id = id
        self.preview = preview
        self.confidence = confidence
        self.chipRoleRaw = chipRole.rawValue
        self.applyMode = mode
    }

    var chipRole: InlineSuggestionChipRole {
        InlineSuggestionChipRole(rawValue: chipRoleRaw) ?? .caretContinuation
    }

    func asInlineSuggestion() -> InlineSuggestion {
        InlineSuggestion(id: id, preview: preview, confidence: confidence, mode: applyMode, chipRole: chipRole)
    }

    init(from suggestion: InlineSuggestion) {
        id = suggestion.id
        preview = suggestion.preview
        confidence = suggestion.confidence
        chipRoleRaw = suggestion.chipRole.rawValue
        applyMode = suggestion.mode
    }
}

/// Persisted snapshot of AI assistant panels for one journal entry body. Invalid when `contentFingerprint` ≠ current text+inspiration.
struct JournalAssistantCache: Codable, Equatable, Hashable {
    var contentFingerprint: UInt64
    var updatedAt: Date
    var outlineSections: [JournalAssistantOutlineSnapshot]
    var inlineSuggestions: [JournalAssistantInlineSnapshot]
    var nudgeSuggestions: [JournalAssistantInlineSnapshot]
    var pinnedNudge: JournalAssistantInlineSnapshot?
    var detectedCorrelations: [EmotionCorrelation]
    var suggestedPeople: [JournalSuggestedPerson]
    var correlationClarifiers: [JournalCorrelationClarifier]
}
...
final class JournalInlineSuggestionEngine: ObservableObject {
    ...
    private let telemetry = JournalSuggestionTelemetry.shared
    /// When non-nil, debounced refresh should not wipe assistant UI if the model returns empty for the same body text.
    private var assistantRestoredContentFingerprint: UInt64?
    /// Called after suggestions, outline, or nudges change so the editor can persist `entry.assistantCache`.
    var onAssistantStateMayNeedPersistence: (() -> Void)?
    ...
    /// Restores assistant UI from `JournalEntry.assistantCache` when fingerprints match (e.g. after iCloud sync or reopening).
    func restoreAssistantCache(_ cache: JournalAssistantCache, text: String, inspiration: String) {
        let fp = JournalAssistantFingerprint.combine(content: text, inspiration: inspiration)
        guard cache.contentFingerprint == fp else { return }

        let outline = cache.outlineSections.map { snap in
            JournalOutlineSection(
                id: snap.id,
                label: snap.label,
                themes: snap.themes,
                textRange: NSRange(location: snap.textRangeStartUTF16, length: snap.textRangeLengthUTF16)
            )
        }
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        var outlineHasher = Hasher()
        outlineHasher.combine(trimmed)
        let outlineMemHash = UInt64(bitPattern: Int64(outlineHasher.finalize()))

        withAnimation(.easeOut(duration: 0.12)) {
            backboneOutline = outline
            suggestions = cache.inlineSuggestions.map { $0.asInlineSuggestion() }
            if let p = cache.pinnedNudge {
                pinnedNudge = p.asInlineSuggestion()
                nudgeSuggestions = []
            } else {
                pinnedNudge = nil
                nudgeSuggestions = cache.nudgeSuggestions.map { $0.asInlineSuggestion() }
            }
        }
        backboneCache = outline.isEmpty ? nil : (outlineMemHash, outline)
    ...
    func textDidChange(_ text: String, selection: NSRange, referenceDate: Date, isFitnessReport: Bool, inspirationContext: String, isComposing: Bool = false) {
        ...
        let fpNow = JournalAssistantFingerprint.combine(content: text, inspiration: inspirationContext)
        ...
        debounceTask = Task { @MainActor in
            ...
            var nextSuggestions = Array(deduped.prefix(6))
            var nextNudges: [InlineSuggestion] = []
            if pinnedNudge == nil {
                nextNudges = Array(
                    deduplicateSuggestions(newNudges, existingText: text)
                        .sorted { $0.confidence > $1.confidence }
                        .prefix(4)
                )
            }

            let fpAfter = JournalAssistantFingerprint.combine(content: text, inspiration: inspirationContext)
            if assistantRestoredContentFingerprint == fpAfter {
                if nextSuggestions.isEmpty, !suggestions.isEmpty {
                    nextSuggestions = suggestions
                }
                if pinnedNudge == nil, nextNudges.isEmpty, !nudgeSuggestions.isEmpty {
                    nextNudges = nudgeSuggestions
                }
            }
            ...
            telemetry.trackShown(inline: suggestions.count, nudges: nudgeSuggestions.count)
            onAssistantStateMayNeedPersistence?()
        }
    }
    ...
    private func heuristicBackbone(text: String) -> [JournalOutlineSection] {
        ...
        return paragraphs.enumerated().map { idx, para in
            ...
            return JournalOutlineSection(id: UUID(), label: label, themes: [], textRange: actualRange)
        }
    }
    ...
    private func mapBackboneSectionsToRanges(aiSections: [JournalBackboneSection], fullText: NSString) -> [JournalOutlineSection] {
        ...
        return aiSections.enumerated().map { idx, sec in
            ...
            offset = end
            return JournalOutlineSection(id: UUID(), label: sec.label, themes: sec.themes, textRange: sectionRange)
        }
    }
    #endif
    ...
    func apply(_ suggestion: InlineSuggestion, to text: String, selection: NSRange) -> (String, NSRange) {
        ...
        case .completion(let insert):
            ...
            telemetry.trackAccepted(role: suggestion.chipRole)
    ...
    private func deduplicateSuggestions(_ suggestions: [InlineSuggestion], existingText: String) -> [InlineSuggestion] {
        ...
        return suggestions.filter { s in
            let normalized = normalizeSuggestionText(s.preview)
    ...
    private func normalizeSuggestionText(_ text: String) -> String {
        ...
        let stripped = lower.components(separatedBy: CharacterSet.alphanumerics.inverted).joined(separator: " ")
        let stopWords: Set<String> = ["the", "a", "an", "and", "to", "of", "in", "for", "on", "at", "is", "are"]
        return stripped
    ...
    private func buildSuggestions(ctx: SuggestionFocusContext, referenceDate: Date, isFitnessReport: Bool) -> [InlineSuggestion] {
        ...
        results.append(contentsOf: pathAwareNudges(ctx: ctx))
    ...
    private func pathAwareNudges(ctx: SuggestionFocusContext) -> [InlineSuggestion] {
        guard let quest = PathQuestStore.shared.activeQuest else { return [] }
        let focus = quest.associations.prefix(2)
        guard !focus.isEmpty else { return [] }
        let prompt = "How did today's choices move \(focus.map { $0.displayName.lowercased() }.joined(separator: " + ")) forward?"
        let prefix = Self.newlinePrefixForStandaloneInsert(fullText: ctx.fullText, cursorUTF16: ctx.cursorUTF16)
        return [
            InlineSuggestion(
                preview: prompt,
                confidence: 0.66,
                mode: .completion(insertUTF16: prefix + prompt),
                chipRole: .nudge
            )
        ]
    }
...
struct JournalEditorView: View {
    ...
    private func saveJournalAssistantCacheToEntry(force: Bool = false) {
        let now = Date()
        if !force, now.timeIntervalSince(lastJournalAssistantCachePersist) < 1.0 {
            return
        }
        ...
        let outlineSnaps = suggestionEngine.backboneOutline.map { sec in
            JournalAssistantOutlineSnapshot(
                ...
                label: sec.label,
                themes: sec.themes,
                textRangeStartUTF16: sec.textRange.location,
                textRangeLengthUTF16: sec.textRange.length
            )
        }
        entry.assistantCache = JournalAssistantCache(
            contentFingerprint: fp,
            updatedAt: Date(),
            outlineSections: outlineSnaps,
            inlineSuggestions: suggestionEngine.suggestions.map { JournalAssistantInlineSnapshot(from: $0) },
            nudgeSuggestions: suggestionEngine.nudgeSuggestions.map { JournalAssistantInlineSnapshot(from: $0) },
            pinnedNudge: suggestionEngine.pinnedNudge.map { JournalAssistantInlineSnapshot(from: $0) },
            detectedCorrelations: correlationEngine.detectedCorrelations,
            suggestedPeople: correlationEngine.suggestedPeople,
            correlationClarifiers: correlationEngine.correlationClarifiers
        )
    }
    ...
    private func closeJournalEditor() {
        ...
        #endif
        // Drive dismissal from the parent binding only; mixing `dismiss()` with `sheet(isPresented:)` often breaks on Catalyst when nested under other navigation hosts.
        DispatchQueue.main.async {
            isPresented = false
        }
    }

    private func insertHealthSnapshotIntoEntry() {
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar {
                #if targetEnvironment(macCatalyst)
                // `cancellationAction` / `confirmationAction` often fail to receive taps in Catalyst sheets; use explicit bar placements.
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    HStack(spacing: 10) {
                        Button {
                            insertHealthSnapshotIntoEntry()
                        } label: {
                            Image(systemName: "heart.text.square")
                        }
                        .buttonStyle(.bordered)
                        .help("Insert Health Snapshot")

                        Button("Save") {
                            saveJournalEntryAndClose()
                        }
                        .buttonStyle(.borderedProminent)
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            ForEach(correlationEngine.suggestedPeople) { person in
                HStack(spacing: 10) {
                    ...
                    Spacer()
                    if let existing = entry.personProfiles[person.name.lowercased()]?.relationship ?? entry.personRelationshipHints[person.name.lowercased()] {
                    ...
                    Menu {
                        Button("Same person as memory") {
                            let key = person.name.lowercased()
                            if let profile = entry.personProfiles[key] {
                                entry.personRelationshipHints[key] = profile.relationship
                            }
                        }
                        Button("Different person (add note)") {
                            let key = person.name.lowercased()
                            var profile = entry.personProfiles[key] ?? JournalPersonProfile(name: person.name, relationship: "other")
                            profile.disambiguationNote = "Different person, add qualifier (e.g. last name / role)."
                            entry.personProfiles[key] = profile
                        }
                        Divider()
                        ForEach(["Friend", "Family", "Teacher", "Classmate", "Coworker", "Partner", "Mentor", "Other"], id: \.self) { role in
                            Button {
                                withAnimation {
                                    let key = person.name.lowercased()
                                    entry.personRelationshipHints[key] = role.lowercased()
                                    var profile = entry.personProfiles[key] ?? JournalPersonProfile(name: person.name, relationship: role.lowercased())
                                    profile.relationship = role.lowercased()
                                    entry.personProfiles[key] = profile
                                }
                            } label: {
                                Text(role)
                    ...
                    if let profile = entry.personProfiles[person.name.lowercased()] {
                        HStack(spacing: 6) {
                            Text("Known as \(profile.relationship)")
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                            if !profile.disambiguationNote.isEmpty {
                                Text("(\(profile.disambiguationNote))")
                                    .font(.caption2)
                                    .foregroundStyle(.secondary)
                            }
                        }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(correlationEngine.correlationClarifiers) { clarifier in
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    let stableKey = clarifier.stableKey
            ...
            HStack(alignment: .top, spacing: 8) {
                ForEach(suggestions) { suggestion in
                    Button {
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 20, style: .continuous)
                                .stroke(stroke.opacity(0.35), lineWidth: 1)
                        )
                    }
                    .buttonStyle(.plain)
                }
    ...
    private enum JournalNudgeCardMetrics {
        /// Wide enough for two abreast on phone; scroll horizontally on narrow widths.
    ...
    private func nudgeCard(_ nudge: InlineSuggestion, onTap: @escaping () -> Void) -> some View {
        Button(action: onTap) {
            ...
            .frame(
                minWidth: JournalNudgeCardMetrics.minWidth,
                maxWidth: JournalNudgeCardMetrics.maxWidth,
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                Button {
                    ...
                    JournalSuggestionTelemetry.shared.trackSavedNudge()
                    suggestionEngine.unpinNudge()
                } label: {
