...
fileprivate struct WatchCompatibleQuestRecord: Codable {
    let id: UUID
...
extension StageQuestRecord {
    fileprivate init(from watch: WatchCompatibleQuestRecord) {
        self.init(
            id: watch.id,
            completedAt: watch.completedAt,
            workoutID: watch.workoutID,
            goalRawValue: watch.goalRawValue,
            roleRawValue: watch.roleRawValue,
            valueMin: watch.valueMin,
            valueMax: watch.valueMax,
...
final class StageQuestStore: ObservableObject {
    ...
    func load() {
        var mergedRecords: [StageQuestRecord] = []
        
        #if targetEnvironment(macCatalyst)
        let localRecords = decodeFromLocal([StageQuestRecord].self, key: recordsKey) ?? []
        mergedRecords.append(contentsOf: localRecords)
        
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            self?.cloud.synchronize()
            let cloudRecords = self?.decodeFromCloud([StageQuestRecord].self, key: self?.recordsKey ?? "") ?? []
            DispatchQueue.main.async {
                self?.mergeAndUpdateRecords(cloudRecords: cloudRecords)
            }
        }
        #else
        cloud.synchronize()
        ...
        mergedRecords.append(contentsOf: cloudRecords)
        mergedRecords.append(contentsOf: localRecords)
        
        if mergedRecords.isEmpty {
            let watchCloudRecords = decodeFromCloud([WatchCompatibleQuestRecord].self, key: recordsKey) ?? []
            let watchLocalRecords = decodeFromLocal([WatchCompatibleQuestRecord].self, key: recordsKey) ?? []
            mergedRecords = (watchCloudRecords + watchLocalRecords).map { watch in
                StageQuestRecord(
                    id: watch.id,
                    completedAt: watch.completedAt,
                    workoutID: watch.workoutID,
        ...
        records = mergedRecords.sorted { $0.completedAt < $1.completedAt }
        recommendations = decode([String: StageRangeRecommendation].self, key: recommendationsKey) ?? [:]
        #endif
    }
    
    #if targetEnvironment(macCatalyst)
    private func mergeAndUpdateRecords(cloudRecords: [StageQuestRecord]) {
        var merged = records
        ...
        records = merged.sorted { $0.completedAt < $1.completedAt }
        recommendations = decode([String: StageRangeRecommendation].self, key: recommendationsKey) ?? [:]
    }
    #endif
...
struct PastQuestsView: View {
    ...
    var body: some View {
        ...
        .onReceiveViewControl(.nutrivanceViewControlFilter1) { selectedWindow = .d7 }
        .onReceiveViewControl(.nutrivanceViewControlFilter2) { selectedWindow = .d28 }
        .onReceiveViewControl(.nutrivanceViewControlFilter3) { selectedWindow = .year }
        .onReceiveViewControl(.nutrivanceViewControlLogNewQuest) { isManualLoggerPresented = true }
        .onReceiveViewControl(.nutrivanceViewControlPastQuestsPrevious) { stepWindow(-1) }
        .onReceiveViewControl(.nutrivanceViewControlPastQuestsNext) {
            guard canStepForward else { return }
            stepWindow(1)
        }
        .onReceiveViewControl(.nutrivanceViewControlToday) { jumpToToday() }
