...
final class CompanionWorkoutLiveManager: NSObject, ObservableObject {
    ...
    private func finalizeLocalWorkoutSave() async {
        ...
        do {
            ...
            let associations: [NutrivanceAssociation] = [.fitness, .physicalOutput, .energyLevels]
            NotificationCenter.default.post(name: .nutrivanceViewControlPathfinderWorkoutCompleted, object: associations)
        } catch {
    ...
    private func attachRecoveredLocalSession(_ session: HKWorkoutSession) {
        ...
        let configuration = HKWorkoutConfiguration()
        configuration.activityType = session.workoutConfiguration.activityType
        configuration.locationType = session.workoutConfiguration.locationType
        builder?.dataSource = HKLiveWorkoutDataSource(healthStore: healthStore, workoutConfiguration: configuration)
