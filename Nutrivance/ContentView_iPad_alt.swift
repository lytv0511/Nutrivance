...
//
//  Alternate sidebar `TabView` shell. Tabbed browser, split panes, and combined navigation titles
//  live in `ContentView_iPad.swift` (used by the main iPad / Catalyst entry point).
...
struct ContentView_iPad_alt: View {
    ...
    var body: some View {
        TabView(selection: $navigationState.selectedRootTab) {
            Tab("Start Page", systemImage: "magnifyingglass", value: RootTabSelection.search, role: .search) {
                SearchView_iPhone()
            }
            ...
            TabSection("Training") {
                ...
                .defaultVisibility(.hidden, for: .tabBar)
                Tab("Readiness Check", systemImage: "heart.fill", value: RootTabSelection.readiness) {
                    ReadinessCheckView()
                }
