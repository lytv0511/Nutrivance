...
extension Notification.Name {
    ...
    static let nutrivanceViewControlPathfinderWorkoutCompleted = Notification.Name("nutrivance.viewControl.pathfinder.workoutCompleted")
    static let nutrivanceViewControlPathfinderMindfulnessLogged = Notification.Name("nutrivance.viewControl.pathfinder.mindfulnessLogged")
    ...
    static let nutrivanceViewControlTrainingCalendarPreviousMonth = Notification.Name("nutrivance.viewControl.trainingCalendar.previousMonth")
    static let nutrivanceViewControlTrainingCalendarNextMonth = Notification.Name("nutrivance.viewControl.trainingCalendar.nextMonth")
    ...
    static let nutrivanceBrowserCloseTab = Notification.Name("nutrivance.browser.closeTab")
    static let nutrivanceBrowserCloseOtherTabs = Notification.Name("nutrivance.browser.closeOtherTabs")
    static let nutrivanceBrowserCloseWindow = Notification.Name("nutrivance.browser.closeWindow")
    static let nutrivanceBrowserSplitAssignLeft = Notification.Name("nutrivance.browser.splitAssignLeft")
    static let nutrivanceBrowserSplitAssignRight = Notification.Name("nutrivance.browser.splitAssignRight")
    static let nutrivanceBrowserSwapWithLastInteracted = Notification.Name("nutrivance.browser.swapWithLastInteracted")
    static let nutrivanceBrowserShowAllTabs = Notification.Name("nutrivance.browser.showAllTabs")
    static let nutrivanceBrowserFocusAddressBar = Notification.Name("nutrivance.browser.focusAddressBar")
    static let nutrivanceBrowserPreviousTab = Notification.Name("nutrivance.browser.previousTab")
    static let nutrivanceBrowserNextTab = Notification.Name("nutrivance.browser.nextTab")
    static let nutrivanceBrowserSwapSplitPanes = Notification.Name("nutrivance.browser.swapSplitPanes")
    static let nutrivanceBrowserCyclePrevTabInPane = Notification.Name("nutrivance.browser.cyclePrevTabInPane")
    static let nutrivanceBrowserCycleNextTabInPane = Notification.Name("nutrivance.browser.cycleNextTabInPane")
    static let nutrivanceBrowserCyclePrevTabGlobally = Notification.Name("nutrivance.browser.cyclePrevTabGlobally")
    static let nutrivanceBrowserCycleNextTabGlobally = Notification.Name("nutrivance.browser.cycleNextTabGlobally")
    static let nutrivanceBrowserCycleSplitPaneForward = Notification.Name("nutrivance.browser.cycleSplitPaneForward")
    static let nutrivanceBrowserCycleSplitPaneReverse = Notification.Name("nutrivance.browser.cycleSplitPaneReverse")
    /// Toggle ⌘⇧F: present or dismiss the browser page full-screen cover (iPad / Catalyst).
    static let nutrivanceBrowserToggleFullscreen = Notification.Name("nutrivance.browser.toggleFullscreen")
    /// ⌘⇧O: Xcode-style “Open Quickly” overlay to open a destination in a new browser tab (iPad / Catalyst shell).
    static let nutrivanceBrowserOpenQuickly = Notification.Name("nutrivance.browser.openQuickly")
    /// ⌘⌥1…9,0: open the corresponding Start Page favorite in the focused browser tab (`userInfo["index"]`: 0…9).
    static let nutrivanceBrowserOpenFavoriteSlot = Notification.Name("nutrivance.browser.openFavoriteSlot")
}
...
class NavigationState: ObservableObject {
    ...
    func isGloballyActiveRootTab(_ tab: RootTabSelection) -> Bool {
        selectedRootTab == tab || browserSplitEmbeddedRootTabs.contains(tab)
    }
...
struct NutrivanceApp: App {
    /// Drives `CommandMenu("View Controls")` from the **key window’s** `NavigationState` (`NutrivanceSceneKeyWindowResolver`).
    @ObservedObject private var menuStateBinder = NutrivanceMenuStateBinder.shared
    ...
    init() {
        ...
        #if canImport(WatchConnectivity)
        _ = iOSWorkoutSyncManager.shared
        #endif
    }
    ...
    var body: some Scene {
        ...
        .commands {
            ...
            CommandMenu("Navigation") {
                Button("Back") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("back")
                }
                .keyboardShortcut("[", modifiers: [.command])

                Button("Dashboard") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("dashboard")
                }
                .keyboardShortcut("D", modifiers: [.command, .shift])

                Button("Today's Plan") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("todaysPlan")
                }
                .keyboardShortcut("P", modifiers: [.command, .shift])

                Button("Training Calendar") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("trainingCalendar")
                }
                .keyboardShortcut("C", modifiers: [.command, .shift])

                Button("Recovery Score") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("recoveryScore")
                }
                .keyboardShortcut("Y", modifiers: [.command, .shift])

                Button("Readiness") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("readiness")
                }
                .keyboardShortcut("I", modifiers: [.command, .shift])

                Button("Strain vs Recovery") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("strainRecovery")
                }
                .keyboardShortcut("V", modifiers: [.command, .shift])

                Button("Workout History") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("workoutHistory")
                }
                ...
                Button("Mindfulness Realm") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("mindfulnessRealm")
                }
                .keyboardShortcut("R", modifiers: [.command, .shift])

                Button("Pathfinder") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("pathfinder")
                }
                .keyboardShortcut("K", modifiers: [.command, .shift])

                Button("Journal") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("journal")
                }
            ...
            CommandMenu("Search") {
                Button("Find") {
                    NutrivanceSceneMenuRouter.postMainMenuCommand("find")
                }
                .keyboardShortcut("F", modifiers: [.command])
            }
            CommandMenu("View Controls") {
                if let nav = menuStateBinder.activeNavigationState, hasContextualControls(for: nav.selectedRootTab) {
                    if nav.selectedRootTab == .strainRecovery {
                        Button("Today") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlToday)
                        }
                        .keyboardShortcut("T", modifiers: [.command, .option])

                        Button("Previous") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlPrevious)
                        }
                        .keyboardShortcut(.leftArrow, modifiers: [.command])

                        Button("Next") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlNext)
                        }
                        ...
                        Button("1W") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlFilter1)
                        }
                        .keyboardShortcut("1", modifiers: [.command])

                        Button("1M") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlFilter2)
                        }
                        .keyboardShortcut("2", modifiers: [.command])

                        Button("1Y") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlFilter3)
                        }
                        ...
                        Button("Refresh Coach Summary") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlRefresh)
                        }
                        .keyboardShortcut("R", modifiers: [.command])

                        Button("Save Coach Summary to Journal") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlSaveToJournal)
                        }
                        .keyboardShortcut("S", modifiers: [.command])
                    }

                    if nav.selectedRootTab == .stress {
                        Button("Today") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlToday)
                        }
                        .keyboardShortcut("T", modifiers: [.command, .option])
                    ...
                    if nav.selectedRootTab == .sleep {
                        ...
                        Button("Year") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlFilter4)
                        }
                        ...
                        Button("Expand/Collapse Stages") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlExpandCollapse)
                        }
                        .keyboardShortcut("E", modifiers: [.command])

                        Button("Wake Alarms") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlSleepWakeAlarms)
                        }
                        .keyboardShortcut("U", modifiers: [.command])
                    }

                    if nav.selectedRootTab == .pastQuests {
                        Button("Today") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlToday)
                        }
                        .keyboardShortcut("T", modifiers: [.command, .option])
                        ...
                        Button("Previous") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlPastQuestsPrevious)
                        }
                        .keyboardShortcut(.leftArrow, modifiers: [.command])

                        Button("Next") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlPastQuestsNext)
                        }
                        .keyboardShortcut(.rightArrow, modifiers: [.command])
                        ...
                        Button("Log New Quest") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlLogNewQuest)
                        }
                        .keyboardShortcut("N", modifiers: [.command, .shift])
                    }

                    if nav.selectedRootTab == .journal {
                        Button("New Journal Entry") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlNewJournalEntry)
                        }
                        .keyboardShortcut("N", modifiers: [.command, .shift])
                    }

                    if nav.selectedRootTab == .pathfinder {
                        Button("Log Emotion") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlPathfinderLogEmotion)
                        }
                        .keyboardShortcut("N", modifiers: [.command, .shift])
                    }

                    if nav.selectedRootTab == .workoutHistory {
                        Button("Refresh Workouts") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlRefreshWorkouts)
                        }
                        .keyboardShortcut("R", modifiers: [.command])

                        Button("HR Zone Settings") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlHRZoneSettings)
                        }
                        .keyboardShortcut("U", modifiers: [.command])
                    }

                    if nav.selectedRootTab == .recoveryScore {
                        Button("1D") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlRecoveryScoreFilter1D)
                        }
                        .keyboardShortcut("1", modifiers: [.command])

                        Button("1W") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlRecoveryScoreFilter1W)
                        }
                        .keyboardShortcut("2", modifiers: [.command])

                        Button("1M") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlRecoveryScoreFilter1M)
                        }
                        .keyboardShortcut("3", modifiers: [.command])
                        ...
                        Button("Refresh") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlRecoveryScoreRefresh)
                        }
                        .keyboardShortcut("R", modifiers: [.command])
                    }

                    if nav.selectedRootTab == .readiness {
                        Button("Refresh") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlReadinessRefresh)
                        }
                        .keyboardShortcut("R", modifiers: [.command])
                    }

                    if nav.selectedRootTab == .dashboard {
                        Button("7 Days") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlChartRange7d)
                        }
                        .keyboardShortcut("1", modifiers: [.command])

                        Button("30 Days") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlChartRange30d)
                        }
                        .keyboardShortcut("2", modifiers: [.command])

                        Button("Feel Good Score") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlChartRangeFeelGood)
                        }
                        .keyboardShortcut("3", modifiers: [.command])
                        ...
                        Button("Display Units") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlDisplayUnits)
                        }
                        .keyboardShortcut("U", modifiers: [.command])

                        Button("Arrange Dashboard") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlArrangeDashboard)
                        }
                        .keyboardShortcut("E", modifiers: [.command])
                    }

                    if nav.selectedRootTab == .trainingCalendar {
                        Button("Today") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlTrainingCalendarToday)
                        }
                        .keyboardShortcut("T", modifiers: [.command, .option])

                        Button("Previous Day") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlTrainingCalendarPreviousDay)
                        }
                        .keyboardShortcut(.leftArrow, modifiers: [.command])

                        Button("Next Day") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlTrainingCalendarNextDay)
                        }
                        .keyboardShortcut(.rightArrow, modifiers: [.command])

                        Button("Previous Month") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlTrainingCalendarPreviousMonth)
                        }
                        .keyboardShortcut(.leftArrow, modifiers: [.command, .option])

                        Button("Next Month") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlTrainingCalendarNextMonth)
                        }
                        ...
                        Button("Refresh Calendar") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlTrainingCalendarRefresh)
                        }
                        .keyboardShortcut("R", modifiers: [.command])

                        Button("HR Zone Settings") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlTrainingCalendarHRZoneSettings)
                        }
                        .keyboardShortcut("U", modifiers: [.command])
                    }

                    if nav.selectedRootTab == .programBuilder {
                        Button("Workout Views") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlWorkoutViews)
                        }
                        .keyboardShortcut("E", modifiers: [.command])

                        Button("Workout Metric Layout") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlWorkoutMetricLayout)
                        }
                        .keyboardShortcut("U", modifiers: [.command])
                    }

                    if nav.selectedRootTab == .heartZones {
                        Button("Today") {
                            nutrivancePostViewControlForKeyWindow(.nutrivanceViewControlToday)
                        }
                        .keyboardShortcut("T", modifiers: [.command, .option])
