...
struct StrainRecoveryView: View {
    ...
    private func strainRecoveryTimeFilterButton(_ filter: TimeFilter) -> some View {
        strainRecoveryTimeFilterButtonCore(filter)
            .catalystDesktopFocusable()
            .catalystFocusablePrimaryAction { selectTimeFilter(filter) }
    }

    private func strainRecoveryTimeFilterButtonCore(_ filter: TimeFilter) -> some View {
        Button {
    ...
    private var strainRecoveryTrainingLoadSection: some View {
        MetricSectionGroup(title: "Training Load") {
            if MacCatalystHealthDataPolicy.isActive {
                Text(MacCatalystHealthDataPolicy.trainingLoadSyncedNotice)
                    .font(.caption)
                    .foregroundStyle(.secondary)
        ...
        NavigationStack {
            ...
            .toolbar {
                ToolbarItemGroup(placement: .topBarTrailing) {
                    ...
                    .catalystDesktopFocusable()
                    .catalystFocusablePrimaryAction { showingSummarySettings = true }
            ...
            .onReceiveViewControl(.nutrivanceViewControlStrainRecoverySettings) {
                guard navigationState.isGloballyActiveRootTab(.strainRecovery) else { return }
                showingSummarySettings = true
            }
    ...
    private func refreshDataFromCloud() {
        ...
        let recoveryStart = calendar.date(byAdding: .day, value: -28, to: window.start) ?? window.start
        let workoutStart = calendar.date(byAdding: .day, value: -10, to: window.start) ?? window.start
        
        if engine.needsRecoveryMetricsCoverage(from: recoveryStart, to: end) {
            Task {
                await engine.ensureRecoveryMetricsCoverage(from: recoveryStart, to: end)
            }
        }
        if engine.needsWorkoutAnalyticsCoverage(from: workoutStart, to: end) {
            Task {
                await engine.ensureWorkoutAnalyticsCoverage(from: workoutStart, to: end)
            }
        }
    }
    #endif
...
private struct StrainRecoveryAISummarySection: View {
    ...
    var body: some View {
        ...
        .onReceiveViewControl(.nutrivanceViewControlRefresh) {
            performRefresh()
        }
        .onReceiveViewControl(.nutrivanceViewControlSaveToJournal) {
            performSaveToJournal()
        }
...
private enum StrainRecoverySummaryPersistence {
    ...
    /// Local AI summary cache can exceed macOS / Catalyst UserDefaults limits (~4 MiB). Store on disk instead.
    private static let localCacheFileName = "strain_recovery_ai_summary_cache_v2.json"
    /// `NSUbiquitousKeyValueStore` values must stay under 1 MiB; skip cloud push when larger.
    ...
    private static var localCacheDirectoryURL: URL? {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            .appendingPathComponent("Nutrivance", isDirectory: true)
    }

    private static var localCacheFileURL: URL? {
        localCacheDirectoryURL?.appendingPathComponent(localCacheFileName, isDirectory: false)
    }

    private static func readLocalCacheData() -> Data? {
        if let url = localCacheFileURL,
    ...
    private static func writeLocalCacheData(_ data: Data) {
        if let dir = localCacheDirectoryURL {
    ...
    private static func pushCacheToUbiquitousStore(_ encoded: Data, cloudStore: NSUbiquitousKeyValueStore) {
        if encoded.count <= ubiquitousKVSMaxSafeBytes {
    ...
    static func load(forceReload: Bool = false) -> [String: StrainRecoverySummaryCacheEntry] {
        ...
        if let localData = readLocalCacheData(),
    ...
    static func save(
        ...
        inMemoryCache = pruned
        writeLocalCacheData(encoded)
        pushCacheToUbiquitousStore(encoded, cloudStore: cloudStore)
    }
...
// Thin wrapper around the shared pipeline so call sites keep their existing names.
// The previous local implementation rebuilt `Dictionary(uniqueKeysWithValues: ...)` per
// call – now we either accept a precomputed context (preferred) or fall back to the
// engine-based shared variant.
@MainActor
private func recoveryScore(
    for day: Date,
    engine: HealthStateEngine
) -> Double? {
    sharedRecoveryScore(for: day, engine: engine)
}
...
private func recoveryScore(
    ...
    sharedRecoveryScore(for: day, context: context)
}
...
// Routes through `sharedDailyLoadSnapshots` so the file no longer carries a duplicate
// O(days * W) implementation. Mapped into the local `WorkoutSummarySnapshot` so all
...
private func dailyLoadSnapshots(
    ...
    sharedDailyLoadSnapshots(
        ...
        WorkoutSummarySnapshot(
            date: shared.date,
            sessionLoad: shared.sessionLoad,
            totalDailyLoad: shared.totalDailyLoad,
            acuteLoad: shared.acuteLoad,
            chronicLoad: shared.chronicLoad,
            acwr: shared.acwr,
            strainScore: shared.strainScore,
            workoutCount: shared.workoutCount,
            activeDaysLast28: shared.activeDaysLast28,
            daysSinceLastWorkout: shared.daysSinceLastWorkout
        )
    }
}
...
private func periodScoreContext(
    ...
    let recoveryByDay: [Date: Double] = Dictionary(
        uniqueKeysWithValues: recoverySeries.map { (calendar.startOfDay(for: $0.0), $0.1) }
    )
...
private func precomputeCoachPayload(
    ...
    // Build the recovery/readiness context **once** for this payload pass; all per-day
    // score helpers below reuse it instead of rebuilding `dailyHRV` dictionaries.
    let context = RecoveryComputationContext.make(engine: engine)
    ...
    if timeFilter == .day {
        ...
        if let yesterdayRecovery = recoveryScore(for: yesterday, context: context) {
    ...
    let strainSeries = loadSnapshots.map { ($0.date, $0.strainScore) }
    // O(1) day -> strain lookup so the readiness compose loop is not O(days * loadSnapshots).
    let strainByDay: [Date: Double] = Dictionary(
        uniqueKeysWithValues: loadSnapshots.map { (calendar.startOfDay(for: $0.date), $0.strainScore) }
    )
    let recoverySeries = dateSequence(from: reportPeriod.start, to: reportPeriod.end).compactMap { day -> (Date, Double)? in
    ...
    // Reuse the recovery scores we just computed instead of recomputing them inside the readiness loop.
    let recoveryByDay: [Date: Double] = Dictionary(
        uniqueKeysWithValues: recoverySeries.map { (calendar.startOfDay(for: $0.0), $0.1) }
    )
    let readinessSeries = dateSequence(from: reportPeriod.start, to: reportPeriod.end).compactMap { day -> (Date, Double)? in
    ...
    case .week:
        ...
        if suggestionID == "1w-svr" {
            ...
            let ctx28dRecovery = dateSequence(from: past28dStart, to: calendar.date(byAdding: .day, value: -1, to: window.start) ?? window.start).compactMap { day -> Double? in recoveryScore(for: day, context: context) }
...
struct RecoveryScoreSection: View {
    ...
    private var isSuppressedRelativeToSevenDayAverage: Bool {
        ...
        let priorScores = (1...7).compactMap { offset -> Double? in
            ...
            return recoveryScore(for: sourceDay, context: context)
        }
