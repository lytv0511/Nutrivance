...
struct DashboardView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onReceiveViewControl(.nutrivanceViewControlChartRange30d) {
                guard navigationState.isGloballyActiveRootTab(.dashboard) else { return }
                selectedChartRange = .month
            }
            .onReceiveViewControl(.nutrivanceViewControlChartRangeFeelGood) {
                guard navigationState.isGloballyActiveRootTab(.dashboard) else { return }
                activeDetailView = .feelGood
            }
            .onReceiveViewControl(.nutrivanceViewControlDisplayUnits) {
                showUnitSettings = true
                let impact = UIImpactFeedbackGenerator(style: .medium)
                impact.impactOccurred()
            }
            .onReceiveViewControl(.nutrivanceViewControlArrangeDashboard) {
    ...
    private func trainingLoadSelectionOverlay(
        ...
        Rectangle()
            ...
            .dashboardKeyboardFocusable()
            .accessibilityLabel("Training load chart, drag to select a day")
    }
