        ...
        GeometryReader { geometry in
            ...
            VStack(alignment: .leading, spacing: isCompactScreen ? 3 : 4) {
                if let stage = manager.currentMicroStage {
                    ...
                    TargetMetricWithAnimatedStatus(
                        stage: stage,
                        manager: manager,
                        isCompactScreen: isCompactScreen
                    )
...
// MARK: - Animated Status Indicator Components

private struct AnimatedChevronRight: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Animated chevron with smooth looping
            Image(systemName: "chevron.right.2")
                .font(.system(size: size * 0.45, weight: .bold))
                .foregroundStyle(.green)
                .offset(x: isAnimating ? size * 0.3 : -size * 0.3)
...
private struct AnimatedChevronUp: View {
    @State private var offsetY: CGFloat = 0
    let size: CGFloat
    let isWarning: Bool // true for yellow (5 sec), false for red (>5 sec)
    
    var body: some View {
        Image(systemName: "chevron.up.2")
            .font(.system(size: size * 0.6, weight: .bold))
            .foregroundStyle(isWarning ? .yellow : .red)
            .offset(y: offsetY)
            .frame(width: size, height: size)
            .onAppear {
...
private struct AnimatedChevronDown: View {
    @State private var offsetY: CGFloat = 0
    let size: CGFloat
    let isWarning: Bool // true for yellow (5 sec), false for red (>5 sec)
...
private struct TargetMetricWithAnimatedStatus: View {
    let stage: WatchProgramMicroStagePayload
    let manager: WatchWorkoutManager
    let isCompactScreen: Bool
    @State private var timeOutOfRange: TimeInterval = 0
    @State private var lastPositionWasOptimal: Bool = true
    ...
    private var currentPosition: TargetPosition {
        ...
        // For work phases, exceeding the upper bound is still making progress
        // (staying above minimum is what matters for work intensity)
        if role == .work {
            return value >= range.min ? .optimal : .low
        }
        
        // For other phases, going above max needs adjustment
        if value > range.max { return .high }
        return .optimal
    }
    
    private var isCompleteObjective: Bool {
        switch goal {
        case .time, .energy, .distance:
            return true
        default:
            return false
        }
    ...
    var body: some View {
        VStack(spacing: 6) {
            // Large metric display with chevron indicator
            HStack(alignment: .center, spacing: 4) {
                // Main metric value inline with icon
                metricDisplayView()
                
                Spacer()
                
                // Chevron indicator - smaller and inward
                ZStack {
                    if isCompleteObjective {
                        Image(systemName: "checkmark.circle.fill")
                            .font(.system(size: isCompactScreen ? 28 : 34, weight: .bold))
                            .foregroundStyle(.green)
                    } else if currentPosition == .optimal {
                        // Regained in-zone position: reset tolerance and show progress
                        AnimatedChevronRight(size: isCompactScreen ? 24 : 30)
                            .onAppear { timeOutOfRange = 0; lastPositionWasOptimal = true }
                    } else if currentPosition == .low {
                        // Below target: show up direction with appropriate warning color
                        AnimatedChevronUp(
                            size: isCompactScreen ? 24 : 30,
                            isWarning: timeOutOfRange < 5
                        )
                    } else {
                        // Above target: show down direction with appropriate warning color
                        AnimatedChevronDown(
                            size: isCompactScreen ? 24 : 30,
                            isWarning: timeOutOfRange < 5
                        )
                ...
                .frame(width: isCompactScreen ? 32 : 40, height: isCompactScreen ? 32 : 40)
                .padding(.trailing)
                .padding(.trailing)
                .onChange(of: currentPosition) { oldPosition, newPosition in
                    // When transitioning TO optimal, reset the tolerance
                    if newPosition == .optimal && oldPosition != .optimal {
                        timeOutOfRange = 0
                        lastPositionWasOptimal = true
                    } else if newPosition != .optimal && oldPosition == .optimal {
                        // Transitioning away from optimal, start tracking out-of-range time
                        lastPositionWasOptimal = false
                        timeOutOfRange = 0
                    }
                }
            }
        ...
        .onReceive(
            ...
            updateOutOfRangeTime()
        }
    }
    
    @ViewBuilder
    private func metricDisplayView() -> some View {
        let primaryFontSize: CGFloat = isCompactScreen ? 22 : 26
        let secondaryFontSize: CGFloat = isCompactScreen ? 13 : 15
        let iconFontSize: CGFloat = isCompactScreen ? 14 : 16
        switch goal {
        case .heartRateZone:
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        HStack(spacing: 2) {
                            Text("\(Int(watts.rounded()))")
                                ...
                                .fontWidth(.condensed)
                            Text("W")
                                .font(.system(size: iconFontSize, weight: .bold, design: .rounded))
                        }
                    }
                }
            }
    ...
    private func updateOutOfRangeTime() {
        if currentPosition == .optimal {
            timeOutOfRange = 0
        } else {
            timeOutOfRange += 0.1
        }
